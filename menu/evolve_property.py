...
class EvolvePropertyWindow(Frame):
    def __init__(self, master, evolve_property_settings, func):
        ...
        self.which_prop_label = Label(self, text="Fitness function:", font=BODY_FONT, fg=BODY_COLOR)
        self.which_prop_label.grid(row=0, column=0, pady=spacing)

        self.which_prop = StringVar()
        self.which_prop_menu = OptionMenu(self, self.which_prop, *GLOBAL_STATS_NAMES)
        ...
        self.num_gen_label = Label(self, text="Number of generations:", font=BODY_FONT, fg=BODY_COLOR)
        ...
        self.num_gen_entry = Entry(self, width=3, validate="all", validatecommand=is_int_vcmd, font=BODY_FONT, fg=BODY_COLOR)
        self.num_gen_entry.grid(row=1, column=1, columnspan=3, pady=spacing)

        # Range of steps to calculate fitness from
        self.equi_range_label = Label(self, text="Significant step range:", font=BODY_FONT, fg=BODY_COLOR)
        self.equi_range_label.grid(row=2, column=0, pady=spacing)

        self.equi_range_entry0 = Entry(self, width=3, validate="all", validatecommand=is_int_vcmd, font=BODY_FONT, fg=BODY_COLOR)
        self.equi_range_entry0.grid(row=2, column=1, sticky="e", pady=spacing)

        self.equi_range_labelsub = Label(self, text="-", font=BODY_FONT, fg=BODY_COLOR)
        self.equi_range_labelsub.grid(row=2, column=2, sticky="we", pady=spacing)

        self.equi_range_entry1 = Entry(self, width=3, validate="all", validatecommand=is_int_vcmd, font=BODY_FONT, fg=BODY_COLOR)
        self.equi_range_entry1.grid(row=2, column=3, sticky="w", pady=spacing)


        # Buttons
        self.default_button = Button(self, text="Default", width=7, command=self.default)
        self.evolve_button = Button(self, text="Evolve!", width=7, command=self.evolve)
    ...
    def set_values(self, settings):
        ...
        self.num_gen_entry.delete(0,END)
        self.num_gen_entry.insert(0,settings["num_gen"])
        self.equi_range_entry0.delete(0,END)
        self.equi_range_entry0.insert(0,settings["equi_range"][0])
        self.equi_range_entry1.delete(0,END)
